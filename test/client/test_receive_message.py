...
RAW_XML_DATA = """<?xml version="1.0" ?>
                ...
                """
BASE64_XML_DATA = """<?xml version="1.0" ?>
                    <Message xmlns="http://mns.aliyuncs.com/doc/v1">
...
class ReceiveMessageTest(unittest.TestCase):
    ...
    def test_receive_base64_message_not_decode(self):
        # 构建 request, 模拟 不进行base64解码 时, receive_message 对于 经过base64编码的消息体 的 decode()结果
        message_body = "I am 测试字符串."
        queue_name = "test_queue"
        req = ReceiveMessageRequest(queue_name, False)
        resp = ReceiveMessageResponse()
        data = RecvMessageDecoder.decode(BASE64_XML_DATA, req.base64decode)
        MNSClient.make_recvresp(None, data, resp)
        # 不进行 base64解码时，receive_message 的 decode() 结果为 原base64编码字符串
        self.assertEqual(base64.b64encode(message_body.encode("utf-8")).decode("utf-8"), resp.message_body)

    def test_receive_base64_message_decode(self):
        # 构建 request, 模拟 进行base64解码 时, receive_message 对于 经过base64编码的消息体 的 decode()结果
        message_body = "I am 测试字符串."
        queue_name = "test_queue"
        ...
        data = RecvMessageDecoder.decode(BASE64_XML_DATA, req.base64decode)
        MNSClient.make_recvresp(None, data, resp)
        # 进行 base64解码时，receive_message 的 decode() 结果为 base64解码后的字节串
        self.assertEqual(message_body.encode('utf-8'), resp.message_body)

    def test_receive_raw_message_not_decode(self):
    ...
    def test_receive_raw_message_decode(self):
        # 构建 request, 模拟 进行base64解码 时，receive_message 对于 原始消息体 的 decode()结果
